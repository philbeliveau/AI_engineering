...
2. Query the knowledge base for relevant content:
   - Use `/knowledge:search-knowledge` for persona definitions and role descriptions
   - Use `/knowledge:get-patterns` for agent interaction patterns
   - Use `/knowledge:get-methodologies` for processes the agent should know
   - Use `/knowledge:get-decisions` for decision frameworks the agent uses
